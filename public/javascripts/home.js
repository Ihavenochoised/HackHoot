...
// Ehh your loading icon still never implement ah
...
function checkReady() {
    ...
    if (document.readyState === "complete") {
        setTimeout(function () {
            console.log("Loaded");
            enableButton(start, "Get Answers");
            enableButton(botStart, "Launch Bots");
        });
    }
}
...
function enableButton(button, text) {
    ...
    button.style.backgroundImage = "linear-gradient(0deg, #243ae7, #0875ea)";
...
downloadPDFBtn.addEventListener("click", function () {
    downloadPDF(downloadableContent, {
        filename: kahootContent.title,
        openInNewTab: false,
    });
});
async function downloadPDF(
    element,
    { filename = "document.pdf", openInNewTab = false } = {},
) {
    if (requestPending) {
        haptic.error();
        return;
